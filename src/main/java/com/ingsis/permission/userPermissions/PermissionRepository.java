...
public interface PermissionRepository extends JpaRepository<UserPermissions, UUID> {
    ...
    @Query("""
            ...
            """)
    List<UserPermissions> findByUserIdAndAction(@Param("userId") String userId, @Param("action") String action);
    ...
    @Query("""
            ...
            WHERE p.snippetId = :snippetId
              AND p.userId = :userId
            """)
    void deleteBySnippetIdAndUserId(@Param("snippetId") UUID snippetId, @Param("userId") String userId);
    ...
    @Query("""
            ...
            WHERE p.snippetId = :snippetId
              AND p.action = 'ALL'
            """)
    UserPermissions findBySnippetId(@Param("snippetId") UUID snippetId);
}
